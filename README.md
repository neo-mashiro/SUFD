# A Simple MultiThread Unix File Daemon (SUFD) in C
...
<div>
    <table>
        ...
        <tr>
            ...
            <td>close the file so that no further interactions are permitted</td>
        </tr>
        <tr>
...
First we start the daemon in background, a log file is created. We can play with the file server from multiple terminals using `telnet`. With the delay mode turned on, it is easy to observe the correct reader writer synchronization behaviour.
```bash
$ ./sufd -t 8 -T 24 -D
...
Again let's connect to the file server a few times (in background). This time we send _SIGQUIT_ to stop the server. On receiving _SIGQUIT_, the server attempts to terminate, but it will still wait for busy clients if any, which can be seen from the time difference in the log file.
```shell
$ kill -SIGQUIT 27698
$ cat sufd.log
...
...
On startup, this client reads a configuration file `config.ini` in its root folder _./sample client/_, which has four lines:
```
VSIZE 40
HSIZE 75
RHOST localhost
...
```shell
sshell > ! ls    # a local shell command
sshell > ! date  # yet another local command
sshell > ! exit  # exit the shell

sshell > monitor # a remote request to be handled by the shell server

sshell > ! keepalive # turn on keepalive mode
sshell > fopen test  # a remote request to be handled by the file server
sshell > fread 8 10  # another file manipulation request that follows
...
sshell > ! close  # turn off keepalive mode and disconnect
```
